...
const SCRIPT: Step[] = [
  {
    edge: "confluence->stepfn",
    label: "POST /reviews",
    payload: '{ url: "confluence/CR-8231" }',
    durationMs: 1100,
  },
  {
    edge: "stepfn->intent",
    label: "invoke: resolveIntent",
    payload: '{ cr: "CR-8231" }',
    durationMs: 1100,
  },
  {
    edge: "intent->workspace",
    label: "bootstrap workspace",
    payload: '{ repos: 3, sha: "a17f9c2" }',
    durationMs: 1300,
  },
  {
    edge: "workspace->dynamo",
    label: "state: READY",
    payload: '{ status: "READY" }',
    durationMs: 900,
  },
  {
    edge: "workspace->gateway",
    label: "expose MCP tools",
    payload: '{ tools: ["grep","graph"] }',
    durationMs: 900,
  },
  {
    edge: "gateway->agents",
    label: "fan-out → 7 agents",
    payload: "{ parallel: 7 }",
    durationMs: 1100,
  },
  {
    edge: "agents->gateway",
    label: "tool.call: graph.findCallers",
    payload: '{ symbol: "PaymentSvc" }',
    durationMs: 1100,
  },
  { edge: "workspace->s3", label: "audit.append", payload: '{ size: "2.4 KB" }', durationMs: 900 },
  {
    edge: "agents->gateway",
    label: "tool.call: search.kafkaSchema",
    payload: '{ topic: "orders.v2" }',
    durationMs: 1100,
  },
  { edge: "workspace->s3", label: "audit.append", payload: '{ size: "1.8 KB" }', durationMs: 900 },
  {
    edge: "gateway->decision",
    label: "aggregate findings",
    payload: "{ findings: 12, high: 3 }",
    durationMs: 1200,
  },
  {
    edge: "decision->agents",
    label: "publish risk report",
    payload: '{ risk: "MEDIUM", evidence: 27 }',
    durationMs: 1200,
  },
];
...
export function DataFlowSimulator() {
  ...
  const [log, setLog] = useState<
    { id: number; t: string; label: string; payload: string; color: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    setLog((prev) =>
      [
        { id, t, label: step.label, payload: step.payload, color: edge?.color ?? "var(--primary)" },
        ...prev,
      ].slice(0, 9),
    );
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border bg-background/40 px-5 py-3">
        <div className="flex items-center gap-3">
          <span className="relative flex h-2 w-2">
            <span
              ...
              style={{ background: activeEdge?.color }}
            />
            <span
          ...
          <span className="mono text-xs text-foreground">
            {String(stepIdx + 1).padStart(2, "0")} / {String(SCRIPT.length).padStart(2, "0")}
          </span>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="inline-flex items-center gap-1.5 rounded-md border border-border bg-surface px-3 py-1.5 text-xs hover:border-primary/60"
          >
            {playing ? (
              <>
      ...
      <div className="grid lg:grid-cols-[1.7fr_1fr]">
              ...
              return (
                <g key={n.id}>
                  <rect
                    ...
                    style={{
                      filter: isActive ? `drop-shadow(0 0 12px ${n.color})` : "none",
                      transition: "all 0.3s",
                    }}
                  />
                  <foreignObject x={n.x} y={n.y} width={n.w} height={n.h}>
                    <div className="flex h-full items-center gap-2.5 px-3">
                      <div
                        ...
                        style={{ color: n.color, background: "var(--background)" }}
                      >
                      ...
                      <div className="min-w-0">
                        ...
                        {n.sub && (
                          <div className="mono mt-0.5 truncate text-[9.5px] uppercase tracking-wider text-muted-foreground">
                            {n.sub}
                          </div>
                        )}
                      </div>
        ...
        <div className="border-t border-border bg-background/50 lg:border-l lg:border-t-0">
          ...
          <div className="max-h-[360px] space-y-1.5 overflow-hidden p-3">
            <AnimatePresence initial={false}>
              {log.map((entry) => (
                ...
                >
                  ...
                  <div className="mono mt-1 truncate text-[11px] text-foreground/85">
                    {entry.payload}
                  </div>
                </motion.div>
              ))}
            ...
            {log.length === 0 && (
              <div className="mono py-10 text-center text-[11px] text-muted-foreground">
                awaiting packets…
              </div>
            )}
          </div>
